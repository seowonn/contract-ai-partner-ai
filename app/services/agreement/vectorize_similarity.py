...
async def vectorize_and_calculate_similarity(chunks: List[ArticleChunk], pdf_request: DocumentRequest) -> List[dict]:

    async def process_clause(clause_content: str) -> dict:
        # 1️⃣ OpenAI 벡터화 (비동기적으로 실행)
        clause_vector = await embedding_service.embed_text(clause_content)
    ...
    # 각 조항에 대해 비동기적으로 처리
    tasks = []  # 비동기 작업을 위한 리스트
    for article in chunks:
        for clause in article.clauses:
            if len(clause.clause_content) > 1:  # 1글자 이상인 경우만 처리
                tasks.append(process_clause(clause.clause_content))  # 비동기 처리 추가


    # 비동기적으로 모든 작업을 실행하고 결과를 기다림
    return await asyncio.gather(*tasks)  # tasks에 추가된 모든 작업을 비동기로 실행하고 결과를 기다림